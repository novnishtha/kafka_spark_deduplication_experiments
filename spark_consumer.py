...
try:
    ...
    print("\nâœ“ Spark Session created with Delta Lake support.")
except Exception as e:
    print(f"\nâœ— Error creating Spark Session: {e}")
    sys.exit(1)


# --- Wait for Kafka Topics ---
def wait_for_kafka_topics(max_retries=30, retry_delay=5):
    print(f"\nâ³ Waiting for Kafka topics to be ready...")
    for attempt in range(1, max_retries + 1):
        try:
            ...
            test_df.printSchema()
            print(f"âœ“ Kafka topic '{INPUT_TOPIC}' is ready!")
            return True
        except Exception as e:
            if attempt < max_retries:
                print(f"  âš  Not ready yet, waiting {retry_delay}s...")
...
def write_to_sinks_with_dedup(batch_df, batch_id):
    ...
    if batch_df.isEmpty():
        print("âš  Batch is empty, skipping.")
        return
    ...
    if batch_deduped.isEmpty():
        print("âš  All records were duplicates, skipping writes.")
        batch_df.unpersist()
        return
    ...
    try:
        ...
        print(f"âœ“ Wrote {deduped_count} rows to Kafka")

    except Exception as e:
        print(f"âœ— Error writing to Kafka: {e}")
    ...
    try:
        ...
        try:
            ...
            print(f"âœ“ MERGE complete (deduplicated against historical data)")
            ...
            print(f"âœ“ Created Delta table with {deduped_count} rows")
        ...
        print(f"âœ— Error writing to Delta Lake: {e}")
        traceback.print_exc()
...
try:
    ...
    print(f"âœ“ Connected to Kafka topic '{INPUT_TOPIC}'")
except Exception as e:
    print(f"âœ— Error creating Kafka stream: {e}")
    sys.exit(1)
...
print("âœ“ Data transformation configured.")
...
try:
    ...
    print("\n\nâœ“ Streaming job stopped by user.")
except Exception as e:
    print(f"\n\nâœ— Streaming job failed: {e}")
    sys.exit(1)